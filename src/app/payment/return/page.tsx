      ...
      fallback={
        <div className="min-h-[60vh] flex items-center justify-center">
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-gray-600">Finalizing your payment...</p>
          </div>
        </div>
      }
    >
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const to =
          saveResp.receiptUrl ||
          `/receipt?ref=${encodeURIComponent(reference)}&txref=${encodeURIComponent(
            reference
          )}&amount=${Number(pending.ticket.price)}&seats=${pending.ticket.seats}`;
